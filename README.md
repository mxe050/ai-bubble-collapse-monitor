...
日本株は円、海外株はドルで表示します。通貨が違う金額は直接比べず、評価用FCF利回り、成長率、時価総額に占めるDCF価値の割合で比較します。トヨタとホンダは金融サービスを含む自動取得の連結FCFを直接使わず、公式の非金融事業キャッシュフローをFY2022～FY2026の5年中央値に平準化してDCFへ入力します。金融事業価値は別加算していないため、基準DCFとの差をそのまま割高・バブルとは扱いません。投資持株会社のソフトバンクグループはNAV/SOTP、半導体装置会社は景気循環を通した平準化FCFで再確認する必要があります。
