...
def test_screen_and_proxy() -> None:
    ...
    base_close = result["meta"]["proxy_base_close"]
    latest = result["series"][-1]
    require(abs(latest["ai_overheat_normalized_close"] - base_close * latest["ai_overheat_normalized"] / 100) < .02, "normalized nominal value is inconsistent")
    require(abs(latest["ai_overheat_excluded_close"] - base_close * latest["ai_overheat_excluded"] / 100) < .02, "excluded nominal value is inconsistent")
    summary = result["summary"]
    require(abs(summary["latest_actual_close"] - latest["nikkei_close"]) < .02, "latest actual close is inconsistent")
    require(abs(summary["latest_normalized_close"] - latest["ai_overheat_normalized_close"]) < .02, "latest normalized close is inconsistent")
    require(abs(summary["latest_excluded_close"] - latest["ai_overheat_excluded_close"]) < .02, "latest excluded close is inconsistent")
