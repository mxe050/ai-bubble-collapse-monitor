...
assert.match(appSource, /function renderNikkeiAiContributionProxy\(\)/);
...
setTimeout(async () => {
  ...
  assert.ok(!elements.get("nikkeiAiThreeSummary").innerHTML.includes("未算出"));
  assert.strictEqual(elements.get("nikkeiAiComparisonNotice").hidden, false);
  const nikkeiAiSummaryMarkup = elements.get("nikkeiAiThreeSummary").innerHTML;
  const nikkeiAiSummary = nikkeiAiThreeSeriesPayload.summary || {};
  const nikkeiAiYen = new Intl.NumberFormat("ja-JP", { maximumFractionDigits: 0 });
  const latestActualClose = Number(nikkeiAiSummary.latest_actual_close);
  const latestNormalizedClose = Number(nikkeiAiSummary.latest_normalized_close);
  const latestExcludedClose = Number(nikkeiAiSummary.latest_excluded_close);
  assert.ok(Number.isFinite(latestActualClose));
  assert.ok(Number.isFinite(latestNormalizedClose));
  assert.ok(Number.isFinite(latestExcludedClose));
  assert.match(nikkeiAiSummaryMarkup, /日経平均・実績/);
  ...
  assert.ok(nikkeiAiChart, "日経AI寄与比較チャートを描画する");
