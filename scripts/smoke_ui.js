...
assert.match(appSource, /function renderNikkeiAiStrictBasketV2\(\)/);
assert.match(appSource, /strict-fixed-basket-proxy/);
...
assert.match(appSource, /renderNikkeiAiStrictBasketV2/);
...
if (typeof global.atob !== "function") global.atob = (value) => Buffer.from(String(value), "base64").toString("binary");
...
setTimeout(async () => {
  ...
  assert.ok(elements.get("nikkeiAiThreeSummary").innerHTML.includes("20"));
  assert.strictEqual(elements.get("nikkeiAiComparisonNotice").hidden, false);
  assert.ok(elements.get("nikkeiAiTargetNames").innerHTML.includes("285A"));
  assert.ok(elements.get("nikkeiAiStockRows").innerHTML.includes("285A"));
  assert.ok(elements.get("nikkeiAiQualityNotes").innerHTML.includes("20"));
  const nikkeiAiChart = renderedCharts.find((chart) => chart.target && chart.target.id === "nikkeiAiThreeSeriesChart");
  assert.ok(nikkeiAiChart, "strict Nikkei comparison chart renders");
  assert.strictEqual(nikkeiAiChart.config.data.datasets.length, 3);
  assert.ok(nikkeiAiChart.config.data.labels.length >= nikkeiAiThreeSeriesPayload.actual_series.length);
  assert.ok(nikkeiAiChart.config.data.datasets[1].label.includes("20"));
  assert.ok(nikkeiAiChart.config.data.datasets[2].label.includes("16"));
  assert.ok(nikkeiAiChart.config.data.datasets[1].data.some((value) => Number.isFinite(value)));
  assert.ok(nikkeiAiChart.config.data.datasets[2].data.some((value) => Number.isFinite(value)));
