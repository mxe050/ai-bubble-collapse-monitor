...
assert.match(indexSource, /AI過熱候補の寄与を分ける/);
...
setTimeout(async () => {
  ...
  assert.match(elements.get("nikkeiAiThreeSummary").innerHTML, /日経平均・実績/);
  ...
  assert.strictEqual(nikkeiAiChart.config.data.datasets[1].label, "AI過熱寄与を一般市場並みに置換");
  assert.strictEqual(nikkeiAiChart.config.data.datasets[2].label, "AI過熱候補を除いた残存部分");
